...
class PageElementsController extends Controller
{
    ...
    public function get_active_template($id) 
    {
        return $this->templates()->where('id', $id)?->first();
    }

    public function convert_base64($path) 
    {
    ...
    public function convert_resource_file($id) 
    {
        $src = request()->file($id)->store('pdf-images', 'public');
        $path = public_path('\/storage/' . $src);
        return $this->convert_base64($path);
    }
    ...
    public function templates() 
    {
        return collect([
            [
                ...
                'name' => 'Template1',
                'template' => [
                    'header' => $this->convert_base64(base_path('/public/images/sample-template.png')),
                    'footer' => $this->convert_base64(base_path('/public/images/sample-footer-template.png'))
                ]
            ],
            [
                'id' => 'template2',
                'name' => 'Template2',
                'template' => [
                    'header' => $this->convert_base64(base_path('/public/images/sample-template-1.png')),
                    'footer' => $this->convert_base64(base_path('/public/images/sample-footer-template-1.png'))
                ]
            ],
        ]);
    }
